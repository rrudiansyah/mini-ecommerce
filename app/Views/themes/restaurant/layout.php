...
<section id="menu">
    ...
    <div class="menu-grid <?=$f?'':'hidden-cat'?>" id="cat-<?=$cid?>">
        ...
        <div class="menu-card" <?php if(!empty($p['has_variants'])): ?>onclick="openVariantModal(<?=$p['id']?>,'<?=htmlspecialchars($p['name']??'',ENT_QUOTES)?>',<?=$p['price']?>,'<?=htmlspecialchars($p['image']??'',ENT_QUOTES)?>',<?=htmlspecialchars(json_encode(array_values($p['variants']??[])),ENT_QUOTES)?>)"<?php else: ?>onclick="addToCart(<?=$p['id']?>,'<?=htmlspecialchars($p['name']??'',ENT_QUOTES)?>',<?=$p['price']?>,'<?=htmlspecialchars($p['image']??'',ENT_QUOTES)?>')"<?php endif; ?>>
            <div class="menu-img">
